...
import { fetchRanking, formatDuration, isRankingConfigured, RANKING_POLL_INTERVAL_MS, type RankingEntry } from "./ranking-service";
    ...
    <section className="event-ranking-podium" aria-label="Três primeiros colocados">
      {entries.map((entry, index) => (
        <article className={`event-ranking-podium-card rank-${index + 1}`} key={entry.id}>
          ...
          <h2>{entry.nome}</h2>
          <p>{entry.palavrasEncontradas} palavras</p>
          <strong>{formatDuration(entry.tempoTotalMs)}</strong>
        </article>
      ))}
    </section>
  );
...
function RankingRow({ entry, position }: { entry: RankingEntry; position: number }) {
  return (
    <li className="event-ranking-row">
      ...
      <span>{entry.palavrasEncontradas} palavras</span>
      <strong>{formatDuration(entry.tempoTotalMs)}</strong>
    </li>
  );
}
